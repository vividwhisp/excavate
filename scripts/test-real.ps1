# Test the Excavate research pipeline with REAL providers (Tavily + Gemini).
# Reads GEMINI_API_KEY / TAVILY_API_KEY from backend/.env and forces
# RESEARCH_MODE=real so the live providers are exercised.
# Usage:  powershell -ExecutionPolicy Bypass -File scripts/test-real.ps1
...
if (-not (Test-Path $EnvFile)) {
    Fail "backend/.env not found - add GEMINI_API_KEY and TAVILY_API_KEY"
}
if (-not (Select-String -Path $EnvFile -Pattern "GEMINI_API_KEY=.+").Matches.Count) {
    Fail "GEMINI_API_KEY is empty in backend/.env"
}
if (-not (Select-String -Path $EnvFile -Pattern "TAVILY_API_KEY=.+").Matches.Count) {
    Fail "TAVILY_API_KEY is empty in backend/.env"
}

$BinDir = Join-Path $Backend "bin"
$Base = "http://localhost:8080"
$Cookie = Join-Path $env:TEMP "excavate_real_cookies_$PID.txt"
$Tmp = Join-Path $env:TEMP "excavate_real"
...
# ---- 2. Backend (forced real providers) ------------------------------------
$startedServer = $false
if (Get-NetTCPConnection -LocalPort 8080 -State Listen -ErrorAction SilentlyContinue) {
    ...
    Write-Host "Backend started (PID $($proc.Id), real providers)." -ForegroundColor Green
}
...
# ---- 4. Run the real research flow -----------------------------------------
Write-Host "== Running real research flow (Tavily + Gemini) ==" -ForegroundColor Cyan
$email = "real-$PID@test.com"
...
$r = PostJson "/api/threads" "{`"title`":`"Real provider test`"}"
if ($r.code -ne "201") { Check "create thread" $false "status $($r.code): $($r.body)" }
    ...
    if ($r.code -ne "201") { Check "post message" $false "status $($r.code): $($r.body)" }
        ...
        $job = Start-Job -ScriptBlock {
            & curl.exe -s -N -b $using:Cookie --max-time 120 $using:StreamUrl -o $using:StreamFile
        }
        $term = "pending"
        for ($i = 0; $i -lt 120; $i++) {
        ...
        if ($ownerMsgs2) {
            ...
            Check "real sources found" (($ownerMsgs2.sources.Count -gt 0)) "count=$($ownerMsgs2.sources.Count)"
            if ($ownerMsgs2.sources.Count -gt 0) {
                $firstUrl = $ownerMsgs2.sources[0].url
                Check "source has url" ($firstUrl -match "^https?://") $firstUrl
            }
        }
    }
}
...
Remove-Item Env:RESEARCH_MODE -ErrorAction SilentlyContinue
if ($startedServer) {
